...
function setDynamicZoomSpeed(controls: OrbitControlApi, camera: THREE.Camera): void {
  controls.zoomSpeed = zoomSpeedForDistance(camera.position.distanceTo(controls.target));
}

function configureControls(controls: OrbitControlApi, camera: THREE.Camera): void {
  setDynamicZoomSpeed(controls, camera);
...
function configureShadowUpdates(gl: THREE.WebGLRenderer, autoUpdate: boolean): void {
...
export function useViewportPerformance(
  ...
  useEffect(() => {
    ...
    if (!entry) {
      ...
      setPixelRatio(gl, preferredPixelRatio());
      configureShadowUpdates(gl, false);
    }
    ...
    return () => {
      ...
      configureShadowUpdates(current.gl, current.previousShadowAutoUpdate);
